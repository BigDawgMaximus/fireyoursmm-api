...
export async function analyzeVoice(
  ...
  // Save to vault
  await vault.writeFile(clientId, `voice/${role}_voice_profile.md`, profile);

  return profile;
}

export async function analyzeAspirationVoice(
  clientId: string,
  role: string,
  aspirationPosts: string[],
): Promise<string> {
  // Load existing voice profile
  const existingProfile = await vault.readFile(clientId, `voice/${role}_voice_profile.md`);

  const postsText = aspirationPosts.map((p, i) => `--- Aspiration Post ${i + 1} ---\n${p}`).join("\n\n");

  const aspirationAnalysis = await callAgent({
    model: "sonnet",
...
Output: An updated voice profile document (markdown) that blends current + aspirational.`,
  ...
  await vault.writeFile(clientId, `voice/${role}_voice_profile.md`, aspirationAnalysis);

  return aspirationAnalysis;
}

export async function extractTrainingPatterns(
  clientId: string,
  posts: string[],
): Promise<void> {
  const postsText = posts.map((p, i) => `--- Post ${i + 1} ---\n${p}`).join("\n\n");
  ...
  await vault.writeFile(clientId, "training/hooks_that_worked.md", `# Hooks That Worked\n\n${hooks}`);

  // Extract triggers
  const triggers = await callAgent({
    model: "haiku",
  ...
  await vault.writeFile(clientId, "training/triggers_that_worked.md", `# Triggers That Worked\n\n${triggers}`);

  // Extract format patterns
  const formats = await callAgent({
    model: "haiku",
  ...
  await vault.writeFile(clientId, "training/formats_that_worked.md", `# Formats That Worked\n\n${formats}`);

  // Save raw posts
  await vault.writeFile(clientId, "training/best_performing_posts.md", `# Best Performing Posts\n\n${postsText}`);
}
