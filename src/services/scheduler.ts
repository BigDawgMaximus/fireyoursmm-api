...
import { generateIntelligenceBrief } from "./intelligence.js";
import { runContentResearch } from "./autoresearch.js";
import { vault } from "../vault/manager.js";
import { scraper } from "./scraper-client.js";
import type { Platform } from "../types/index.js";

// ============================================================
// SCHEDULER
// Background jobs that make FireYourSMM deliver value on autopilot.
// ============================================================
...
function markRunning(job: string): void {
  jobStatuses[job].running = true;
}

function markDone(job: string): void {
...
async function dailyBriefJob(): Promise<void> {
  if (jobStatuses.daily_brief.running) return;
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        const existingBrief = await prisma.generatedContent.findFirst({
          where: {
            user_id: user.id,
            type: "brief",
        ...
        const brief = await generateIntelligenceBrief(user.id, platforms, "marketing");

        // Save to vault
        await vault.writeFile(user.id, `posts/briefs/${dateStr}.md`, brief);
...
async function competitorScrapeJob(): Promise<void> {
  if (jobStatuses.competitor_scrape.running) return;
    ...
    for (const competitor of competitors) {
      try {
        const result = await scraper.scrapePosts(
          competitor.platform as Platform,
          competitor.username,
          50,
        );
        ...
        for (const post of result.posts) {
          ...
          if (post.url) {
            const existing = await prisma.scrapeResult.findFirst({
        ...
        console.log(
          `[SCRAPE] Scraped ${savedCount} posts from @${competitor.username} on ${competitor.platform}`,
        );
        ...
        console.error(
          `[SCRAPE] Failed for @${competitor.username} on ${competitor.platform}:`,
          err,
        );
      }
    }
...
async function creditResetJob(): Promise<void> {
  if (jobStatuses.credit_reset.running) return;
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        console.log(
          `[CREDITS] Reset for user ${user.id}: ${newAmount} credits (${rollover} rolled over)`,
        );
      } catch (err) {
...
async function autoresearchJob(): Promise<void> {
  if (jobStatuses.autoresearch.running) return;
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        if (userHour !== 2) continue;

        // Starter plan: only run on Sundays (weekly)
        if (user.plan === "starter" && userTime.getDay() !== 0) continue;

        // Check if already ran today
        const dateStr = now.toISOString().split("T")[0];
        const existing = await vault.readFile(user.id, `research/autoresearch_${dateStr}.md`);
        if (existing) continue;

        const start = Date.now();
        const result = await runContentResearch(user.id);
        const elapsed = Date.now() - start;

        console.log(
          `[AUTORESEARCH] Ran ${result.total_experiments} experiments for user ${user.id} in ${elapsed}ms. Cost: $${result.cost_usd.toFixed(3)}`,
        );
      } catch (err) {
...
async function rssNewsJob(): Promise<void> {
  if (jobStatuses.rss_news.running) return;
