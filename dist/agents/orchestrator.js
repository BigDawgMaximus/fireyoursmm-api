...
exports.orchestrateDraft = orchestrateDraft;
...
// ============================================================
async function orchestrateDraft(req) {
    ...
    const [voiceProfile, knowledgeBase, training, competitorIntel, algorithmIntel] = await Promise.all([
        manager_js_1.vault.loadVoiceProfile(req.client_id, role),
        manager_js_1.vault.loadKnowledgeBase(req.client_id),
        manager_js_1.vault.loadTrainingData(req.client_id),
        manager_js_1.vault.loadCompetitorIntel(req.client_id),
        manager_js_1.vault.readFile(req.client_id, "meta/algorithm_intel.md"),
    ]);
    const bannedWordsStr = algorithm_js_1.BANNED_WORDS.join(", ");
...
CLIENT ROLE: ${role}
  `.trim();
    ...
    const mainDraftPromise = (0, claude_js_1.callAgent)({
        model: "sonnet",
...
Take the user's raw draft and rewrite it as a high-performing social media post. Apply everything you know about hooks, structure, triggers, and platform optimization.`,
    ...
    const [mainDraft, agentResults] = await Promise.all([
        mainDraftPromise,
        (0, claude_js_1.callAgentsParallel)(agentCalls),
    ]);
    // Now run Platform Agent with the improved draft (needs main draft output)
    const platformResult = await (0, claude_js_1.callAgentJSON)({
        model: "haiku",
    ...
    if (req.retention) {
        ...
        await manager_js_1.vault.writeFile(req.client_id, `posts/drafts/${draftId}.md`, draftContent);
    }
    return {
...
function calculateVelocityScore(hook, structure, trigger, sentiment) {
    ...
    return Math.round(hookScore * hookWeight +
