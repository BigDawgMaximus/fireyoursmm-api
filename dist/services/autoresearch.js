...
exports.runContentResearch = runContentResearch;
const prisma_js_1 = require("../lib/prisma.js");
const manager_js_1 = require("../vault/manager.js");
const claude_js_1 = require("./claude.js");
const algorithm_js_1 = require("../config/algorithm.js");
const experiment_log_js_1 = require("./experiment-log.js");
// ============================================================
...
// ============================================================
const bannedWordsList = algorithm_js_1.BANNED_WORDS.join(", ");
// ============================================================
// MAIN ENTRY POINT
// ============================================================
async function runContentResearch(userId) {
    const start = Date.now();
    let aiCalls = 0;
    // ── Load all context ─────────────────────────────────────
    const [voiceProfile, training, competitorIntel] = await Promise.all([
        manager_js_1.vault.loadVoiceProfile(userId, "marketing"),
        manager_js_1.vault.loadTrainingData(userId),
        manager_js_1.vault.loadCompetitorIntel(userId),
    ]);
    const historicalPosts = await loadHistoricalPosts(userId, 90);
    const calibration = await (0, experiment_log_js_1.getCalibrationFactors)(userId);
    ...
    const topHooks = experiments
        ...
        .slice(0, 3);
    const formats = ["single", "thread", "image_text", "poll"];
    ...
    await Promise.all([
        (0, experiment_log_js_1.saveExperiments)(userId, experiments),
        manager_js_1.vault.writeFile(userId, `research/autoresearch_${dateStr}.md`, formatResearchReport(result)),
    ]);
    ...
    if (hookExps.length > 0) {
        const newHooks = hookExps
            ...
            .join("\n");
        if (newHooks) {
            await manager_js_1.vault.appendFile(userId, "training/hooks_that_worked.md", `\n\n## Autoresearch ${dateStr}\n${newHooks}`);
        }
    }
    return result;
}
function calculateBaseline(posts) {
    ...
    const formatEng = {};
    for (const post of posts) {
        if (!formatEng[post.format])
            formatEng[post.format] = [];
...
function scoreHookAgainstHistory(hook, hookType, baseline, posts) {
    ...
    if (typeAvg > overallAvg * 1.2)
        score += 20; // This type outperforms
    else if (typeAvg > overallAvg)
        score += 10;
    else if (typeAvg < overallAvg * 0.5)
        score -= 15; // This type underperforms
    ...
    if (lengthDiff < 0.2)
        score += 10; // Similar length
    else if (lengthDiff > 0.5)
        score -= 10; // Very different length
    // 3. First-word impact
    const firstWord = hook.split(/\s/)[0]?.toUpperCase() ?? "";
    if (/^\d/.test(firstWord))
        score += 8; // Starts with number
    if (firstWord.endsWith("?"))
        score += 5; // Question
    if (["BREAKING", "JUST", "ALERT", "UNPOPULAR"].includes(firstWord))
        score += 10;
...
function scoreFormatAgainstHistory(format, baseline) {
    let score = 50;
    const formatAvg = baseline.topFormats[format] ?? 0;
    const overallAvg = baseline.avgEngagement || 1;
    if (formatAvg > overallAvg * 1.3)
        score += 25; // This format significantly outperforms
    else if (formatAvg > overallAvg)
        score += 15;
    else if (formatAvg < overallAvg * 0.7)
        score -= 15;
...
function analyzeOptimalTiming(posts) {
    ...
    for (const post of posts) {
        ...
        if (!bucketData[bucket])
            bucketData[bucket] = { total: 0, count: 0 };
    ...
    const dayData = {};
    for (const post of posts) {
        if (!dayData[post.dayOfWeek])
            dayData[post.dayOfWeek] = { total: 0, count: 0 };
...
function analyzeContentPillars(posts, competitorIntel) {
...
function findCompetitorGaps(posts, competitorIntel) {
    const experiments = [];
...
function checkVoiceDrift(voiceProfile, posts) {
    const experiments = [];
    const recent = posts.slice(0, 10);
    if (recent.length === 0 || !voiceProfile)
        return experiments;
        ...
        for (const post of recent) {
            ...
            switch (marker) {
                ...
                case "short_sentences":
                    if (text.split(/[.!?]/).filter(Boolean).length > 2 && text.length < 200)
                        matchCount++;
                    break;
                case "questions":
                    if (text.includes("?"))
...
// ============================================================
async function generateHooksWithType(voiceProfile, trainingHooks, hookType) {
    const response = await (0, claude_js_1.callAgent)({
        model: "haiku",
...
async function expandHookToFormat(voiceProfile, hook, format) {
...
// HELPERS
// ============================================================
async function loadHistoricalPosts(userId, days) {
    const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
    // Load from GeneratedContent (user's own content)
    const generated = await prisma_js_1.prisma.generatedContent.findMany({
        where: {
            user_id: userId,
            type: { in: ["post", "hook"] },
            approved: true,
    ...
    const bestPosts = await manager_js_1.vault.readFile(userId, "training/best_performing_posts.md");
    const posts = [];
...
function detectHookTypes(text) {
    const firstLine = text.split("\n")[0]?.substring(0, 120) ?? "";
    const types = [];
    for (const pattern of algorithm_js_1.HOOK_PATTERNS) {
        if (pattern.template.includes("{") && firstLine.length > 10) {
            // Simple heuristic matching
            if (pattern.name === "urgency_break" && /^(BREAKING|JUST IN|ALERT)/i.test(firstLine))
                types.push("urgency");
            if (pattern.name === "contrarian" && /wrong|unpopular/i.test(firstLine))
                types.push("contrarian");
            if (pattern.name === "number_hook" && /\d+[%xKMB]/.test(firstLine))
                types.push("number");
            if (pattern.name === "binary_choice" && /\bor\b.*\?/i.test(firstLine))
                types.push("binary");
            if (pattern.name === "prediction" && /will |going to/i.test(firstLine))
                types.push("prediction");
            if (pattern.name === "insider" && /nobody|no one|sleeping on/i.test(firstLine))
                types.push("insider");
            if (pattern.name === "listicle" && /^\d+\s+(things?|ways?|tips?)/i.test(firstLine))
                types.push("listicle");
            if (pattern.name === "question" && firstLine.includes("?"))
                types.push("question");
        }
    }
...
// REPORT FORMATTER
// ============================================================
function formatResearchReport(result) {
    const lines = [
        ...
        ...result.top_hooks.map((h, i) => `${i + 1}. [${h.score}/100] **${h.variant}**: ${h.content}`),
        "",
        "## Best Format",
