...
exports.generateWithPatterns = generateWithPatterns;
exports.generateIntelligenceBrief = generateIntelligenceBrief;
...
function buildHookSystemPrompt(voiceProfile, training) {
...
function buildPostSystemPrompt(voiceProfile, platform, training) {
...
function buildRewriteSystemPrompt(voiceProfile) {
...
function buildBriefSystemPrompt(voiceProfile, training) {
...
// ============================================================
async function loadPatterns(clientId, role) {
    const [voiceProfile, training] = await Promise.all([
        manager_js_1.vault.loadVoiceProfile(clientId, role),
...
// ============================================================
async function generateWithPatterns(clientId, prompt, platform, mode, role = "marketing") {
...
// ============================================================
async function generateIntelligenceBrief(clientId, platforms, role = "marketing") {
...
// HELPERS
// ============================================================
async function buildCompetitorDigest(clientId) {
    const competitors = await manager_js_1.vault.listCompetitors(clientId);
    const entries = [];
    for (const handle of competitors) {
        const latestActivity = await manager_js_1.vault.readFile(clientId, `competitors/${handle}/latest_activity.md`);
        const topPosts = await manager_js_1.vault.readFile(clientId, `competitors/${handle}/top_posts.md`);
        if (latestActivity) {
            entries.push(`@${handle}:\n${latestActivity}`);
        }
        else if (topPosts) {
...
async function buildOvernightResearch(clientId) {
    const dateStr = new Date().toISOString().split("T")[0];
    const research = await manager_js_1.vault.readFile(clientId, `research/autoresearch_${dateStr}.md`);
    if (research) {
    ...
    const yesterdayResearch = await manager_js_1.vault.readFile(clientId, `research/autoresearch_${yesterday}.md`);
...
async function buildYesterdayMetrics(clientId) {
    // Check for published posts from yesterday
    const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().split("T")[0];
    const briefFile = await manager_js_1.vault.readFile(clientId, `posts/briefs/${yesterday}.md`);
    if (briefFile) {
