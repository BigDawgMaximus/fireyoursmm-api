...
exports.awardAutoReward = awardAutoReward;
exports.awardReferralSignup = awardReferralSignup;
exports.awardReferralPaid = awardReferralPaid;
const express_1 = require("express");
const auth_js_1 = require("../middleware/auth.js");
const prisma_js_1 = require("../lib/prisma.js");
exports.rewardsRouter = (0, express_1.Router)();
exports.rewardsRouter.use(auth_js_1.requireAuth);
// ============================================================
// REWARD CREDIT VALUES
// ============================================================
const REWARD_CREDITS = {
    ugc_post: 50,
...
// ============================================================
exports.rewardsRouter.get("/", async (req, res) => {
    try {
        const userId = req.user.id;
        const [rewards, totals, pendingCount] = await Promise.all([
            prisma_js_1.prisma.reward.findMany({
                where: { user_id: userId },
                orderBy: { created_at: "desc" },
                take: 50,
            }),
            prisma_js_1.prisma.reward.aggregate({
                where: { user_id: userId, status: { in: ["approved", "auto_approved"] } },
                _sum: { credits: true },
                _count: true,
            }),
            prisma_js_1.prisma.reward.count({
...
// ============================================================
exports.rewardsRouter.get("/referral-code", async (req, res) => {
    try {
        ...
        const referrals = await prisma_js_1.prisma.referral.findMany({
...
// ============================================================
exports.rewardsRouter.post("/claim", async (req, res) => {
    try {
        ...
        const reward = await prisma_js_1.prisma.reward.create({
            data: {
                user_id: userId,
                type,
                credits,
...
// ============================================================
exports.rewardsRouter.get("/leaderboard", async (req, res) => {
    try {
        const monthStart = new Date();
        monthStart.setDate(1);
        monthStart.setHours(0, 0, 0, 0);
        const leaders = await prisma_js_1.prisma.reward.groupBy({
            by: ["user_id"],
            where: {
...
// ============================================================
async function awardAutoReward(userId, type, description) {
    const credits = REWARD_CREDITS[type] ?? 0;
    if (credits === 0)
        return;
    await prisma_js_1.prisma.$transaction([
        prisma_js_1.prisma.reward.create({
            data: {
                user_id: userId,
                type,
                credits,
...
async function awardReferralSignup(referrerCode, referredUserId, referredEmail) {
    ...
    await prisma_js_1.prisma.$transaction([
        prisma_js_1.prisma.referral.create({
            data: {
...
async function awardReferralPaid(referredUserId) {
    const user = await prisma_js_1.prisma.user.findUnique({
    ...
    const existing = await prisma_js_1.prisma.referral.findFirst({
        where: {
            referrer_id: referrer.id,
            referred_user_id: referredUserId,
            status: "paid",
    ...
    await prisma_js_1.prisma.$transaction([
        prisma_js_1.prisma.referral.updateMany({
            where: {
                referrer_id: referrer.id,
                referred_user_id: referredUserId,
            },
